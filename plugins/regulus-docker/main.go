...
import (
	...
	"github.com/networkplayground/plugins/regulus-docker/driver"
...
const (
	...
	// driverSock is the regulus socket for the communication between docker and regulus.
	driverSock = pluginPath + "regulus.sock"
)

var log = l.MustGetLogger("regulus-net-docker-plugin")

func main() {
	app := cli.NewApp()
	app.Name = "regulus-net"
	app.Usage = "regulus Networking Docker Plugin"
...
func run(ctx *cli.Context) {
	...
	if err != nil {
		log.Fatalf("Unable to create regulus-net driver: %s", err)
	}

	log.Info("Regulus Docker plugin ready")
